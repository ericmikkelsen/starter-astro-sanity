...
export const headingType = defineType({
	...
	title: "Heading",
	type: "object",
	fields: [defineField({ name: "text", title: "Text", type: "string", validation: (rule) => rule.required() })],
});
...
export const imageObjectType = defineType({
	...
	fields: [
		...
		defineField({ name: "width", title: "Width", type: "number", validation: (rule) => rule.positive().integer() }),
		defineField({ name: "height", title: "Height", type: "number", validation: (rule) => rule.positive().integer() }),
	],
});